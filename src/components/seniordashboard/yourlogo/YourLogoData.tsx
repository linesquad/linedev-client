...
function YourLogoData({ handleOpenModal, setShowUpdateModal, setSelectedId }: { handleOpenModal: () => void, setShowUpdateModal: (show: boolean) => void, setSelectedId: (id: string) => void }) {
    ...
    <div className="w-full max-w-md">
      ...
      {data.data.length > 0 ? (
        <div className="bg-gray-800 rounded-lg shadow-lg p-4 grid gap-3 mt-4">
          {data.data.map((logo: { _id: string; name: string; image: string }) => (
            <div key={logo._id} className="flex items-center gap-3 p-3 bg-gray-700 rounded-md hover:bg-gray-600 transition duration-300">
